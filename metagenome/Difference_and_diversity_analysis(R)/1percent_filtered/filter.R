...
filter_prevalence_csv <- function(file, cutoff = 0.01, out_dir, microbe_type = NULL) {
  ...
  # 只对细菌文件进行ID过滤，保留以k__Bacteria开头的???
  if (microbe_type == "bacteria") {
    ...
    cat("细菌ID过滤结果: ???", before_filter, "行过滤到", after_filter, "行\n")
  ...
  cat("??? 已完??? 1% 流行率过滤并生成 CSV:", fname, "=> 保留", nrow(df_filtered), "个物种\n")
  cat("原始物种???:", nrow(df), "=> 过滤后物种数:", nrow(df_filtered), "\n")
  cat("输出文件:", out_file, "\n\n")
}
...
for (microbe_type in names(files)) {
  ...
  tryCatch({
    ...
    cat("??? 处理", microbe_type, "时出???:", e$message, "\n")
  })
}
