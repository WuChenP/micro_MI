...
main <- function() {
  ...
  # ======= 新增部分：去掉病毒数据中以“HF”开头的行 =======
  if (!is.null(virus_filtered)) {
    cat("\n步骤2.1: 去除病毒数据中以 'HF' 开头的物种...\n")
    hf_rows <- grepl("^HF", rownames(virus_filtered), ignore.case = FALSE)
    removed_count <- sum(hf_rows)
    virus_filtered <- virus_filtered[!hf_rows, , drop = FALSE]
    cat("已去除以 'HF' 开头的物种数:", removed_count, "\n")
    cat("去除后病毒物种数:", nrow(virus_filtered), "\n")
  }
  # =====================================================
  ...
  cat("输出目录: E:/Python/MI_Analysis/metagenome/data_figures/filtered_data_20percent/\n")
  cat("生成的文件:\n")
