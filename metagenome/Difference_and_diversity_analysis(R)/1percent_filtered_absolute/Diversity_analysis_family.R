...
# ----------------------------
microbe_files <- list(
  fungi    = "E:/Python/MI_Analysis/metagenome/Absolute_abundance_analysis/filtered_data_1percent/family_level_data/fungi_family.xlsx",
  virus    = "E:/Python/MI_Analysis/metagenome/Absolute_abundance_analysis/filtered_data_1percent/family_level_data/virus_family.xlsx",
  archaea  = "E:/Python/MI_Analysis/metagenome/Absolute_abundance_analysis/filtered_data_1percent/family_level_data/archaea_family.xlsx",
  bacteria = "E:/Python/MI_Analysis/metagenome/Absolute_abundance_analysis/filtered_data_1percent/family_level_data/bacteria_family.xlsx"
)

meta_file <- "E:/Python/MI_Analysis/metagenome/Absolute_abundance_analysis/filtered_data_1percent/sample_metadata.xlsx"
output_dir <- "E:/Python/MI_Analysis/metagenome/Absolute_abundance_analysis/filtered_data_1percent/diversity_plots_family/"
...
# 创建指标信息保存文件
# ----------------------------
metrics_info_file <- file.path(output_dir, "family_diversity_metrics_information.xlsx")

# 初始化指标信息列表
metrics_info_list <- list()
...
# 函数：保存指标详细信息
# ----------------------------
save_metrics_information <- function(microbe, alpha_df, beta_results, available_metrics) {
  
  # 创建该微生物的指标信息数据框
  microbe_info <- data.frame()
  ...
  for(metric in available_metrics) {
    if(metric %in% colnames(alpha_df) && !all(is.na(alpha_df[[metric]]))) {
      ...
      # 计算基本统计信息
      control_data <- alpha_df[[metric]][alpha_df$Group == "Control"]
      mi_data <- alpha_df[[metric]][alpha_df$Group == "MI"]
      ...
      metric_info <- data.frame(
        ...
        Control_Min = min(control_data, na.rm = TRUE),
        Control_Max = max(control_data, na.rm = TRUE),
        MI_Mean = mean(mi_data, na.rm = TRUE),
        MI_SD = sd(mi_data, na.rm = TRUE),
        MI_Median = median(mi_data, na.rm = TRUE),
        MI_Min = min(mi_data, na.rm = TRUE),
        MI_Max = max(mi_data, na.rm = TRUE),
        Samples_Control = length(control_data),
        Samples_MI = length(mi_data),
        Data_Reliability = get_reliability_level(metric),
        Notes = get_metric_notes(metric)
      )
      
      microbe_info <- rbind(microbe_info, metric_info)
    }
  }
  ...
  beta_info <- data.frame(
    ...
    Description = "Bray-Curtis dissimilarity based on PCoA",
    Control_Mean = NA,
    Control_SD = NA,
    Control_Median = NA,
    Control_Min = NA,
    ...
    MI_Min = NA,
    MI_Max = NA,
    Samples_Control = beta_results$samples_control,
    Samples_MI = beta_results$samples_mi,
    Data_Reliability = "High",
    Notes = paste0("PERMANOVA: p=", beta_results$p_value, 
                   ", R²=", beta_results$r_squared)
  )
  
  microbe_info <- rbind(microbe_info, beta_info)
...
# 函数：获取指标描述
# ----------------------------
get_metric_description <- function(metric) {
  descriptions <- list(
    Observed = "Number of observed taxa (species richness)",
    Richness = "Species richness (same as Observed)",
    Number = "Number of observed taxa",
    Shannon = "Shannon diversity index (accounts for abundance and evenness)",
    Simpson = "Simpson diversity index (probability that two randomly selected individuals belong to different species)",
    InvSimpson = "Inverse Simpson index (effective number of species)",
    Chao1 = "Chao1 estimator (bias-corrected richness estimator)",
    ACE = "Abundance-based Coverage Estimator (richness estimator for rare species)"
  )
  return(ifelse(metric %in% names(descriptions), descriptions[[metric]], "Unknown metric"))
}

# ----------------------------
# 函数：获取数据可靠性等级
# ----------------------------
get_reliability_level <- function(metric) {
  if(metric %in% c("Chao1", "ACE")) {
    return("Low (based on converted relative abundance data)")
  } else {
    return("High (based on original data)")
  }
}
...
# 函数：获取指标备注
# ----------------------------
get_metric_notes <- function(metric) {
  if(metric == "Chao1") {
    return("Calculated from converted relative abundance data - interpret with caution")
  } else if(metric == "ACE") {
    return("Calculated from converted relative abundance data - EXTREME caution advised")
  } else if(metric %in% c("Observed", "Richness", "Number")) {
    return("Direct count of observed taxa")
  } else {
    return("Standard diversity metric")
  }
}
...
for(microbe in names(microbe_files)){
  ...
  # ----------------------------
  beta_results <- list(
    p_value = ifelse(is.na(pval), "NA", sprintf("%.6f", pval)),
    r_squared = ifelse(is.na(r_squared), "NA", sprintf("%.6f", r_squared)),
    samples_control = sum(ord_df$Group == "Control"),
    samples_mi = sum(ord_df$Group == "MI")
  )
  
  microbe_metrics_info <- save_metrics_information(microbe, alpha_df, beta_results, available_metrics)
  metrics_info_list[[microbe]] <- microbe_metrics_info
...
# 保存所有指标信息到Excel文件
# ----------------------------
if(length(metrics_info_list) > 0) {
  # 合并所有微生物的数据
  all_metrics_info <- do.call(rbind, metrics_info_list)
  
  # 创建Excel工作簿
  wb_metrics <- createWorkbook()
  
  # 添加汇总表
  addWorksheet(wb_metrics, "Summary")
  writeData(wb_metrics, "Summary", all_metrics_info)
  
  # 为每个微生物类型添加单独的工作表
  for(microbe in names(metrics_info_list)) {
    addWorksheet(wb_metrics, microbe)
    writeData(wb_metrics, microbe, metrics_info_list[[microbe]])
  }
  
  # 添加指标说明表
  metric_descriptions <- data.frame(
    Metric_Name = c("Observed", "Richness", "Number", "Shannon", "Simpson", 
                    "InvSimpson", "Chao1", "ACE", "Bray-Curtis"),
    Description = c(
      "Number of observed taxa (species richness)",
      "Species richness (same as Observed)",
      "Number of observed taxa",
      "Shannon diversity index (accounts for abundance and evenness)",
      "Simpson diversity index (probability that two randomly selected individuals belong to different species)",
      "Inverse Simpson index (effective number of species)",
      "Chao1 estimator (bias-corrected richness estimator)",
      "Abundance-based Coverage Estimator (richness estimator for rare species)",
      "Bray-Curtis dissimilarity based on PCoA"
    ),
    Data_Type = c(
      "Count data",
      "Count data", 
    ...
    Reliability = c(
      ...
      "High",
      "High",
      "Low (converted from relative abundance)",
      "Low (converted from relative abundance)",
      "High"
    )
  )
  
  addWorksheet(wb_metrics, "Metric_Descriptions")
  writeData(wb_metrics, "Metric_Descriptions", metric_descriptions)
  
  # 保存Excel文件
  saveWorkbook(wb_metrics, metrics_info_file, overwrite = TRUE)
  message("指标信息已保存到: ", metrics_info_file)
}
...
cat("Metrics information saved to: 'family_diversity_metrics_information.xlsx'\n")
