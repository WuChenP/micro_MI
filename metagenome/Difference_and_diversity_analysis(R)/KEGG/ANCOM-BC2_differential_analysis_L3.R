...
# ----------------------------
otu_files <- list(
  virus = "E:/Python/MI_Analysis/metagenome/KEGG_difference_analysis/kegg_profile_L3.csv"
)

metadata_file = "E:/Python/MI_Analysis/metagenome/KEGG_difference_analysis/sample_metadata.xlsx"

# 输出文件夹
output_dir <- "E:/Python/MI_Analysis/metagenome/KEGG_difference_analysis/ancombc2_results_L3/"
...
for (microbe in names(otu_files)) {
  ...
  if (!is.null(ancombc_res)) {
    ...
    # 保存为 Excel
    output_file <- paste0(output_dir, microbe, "_KEGG_L3_ANCOMBC2_results.xlsx")
    wb <- createWorkbook()
