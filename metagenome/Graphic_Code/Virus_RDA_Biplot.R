...
# 执行RDA分析
rda_result <- rda(abundance_hell ~ Group + Age + Gender + BMI + Smoking + Drinking, 
                  data = env_filtered)
...
# 简化变量名称显示（现在会显示Male、Smoker和Drinker）
biplot_data$Variable <- recode(biplot_data$Variable,
                               ...
                               "GenderFemale" = "Female",
                               "BMI" = "BMI",
                               "SmokingSmoker" = "Smoke_Y",  # 现在会显示吸烟
                               "SmokingNon-smoker" = "Smoke_N",
                               "DrinkingDrinker" = "Drink_Y",  # 现在会显示饮酒
                               "DrinkingNon-drinker" = "Drink_N")
...
cat("Female: Female\n")
cat("Smoke_Y: Smoker (reference: Non-smoker)\n")
cat("Smoke_N: Non-smoker\n")
cat("Drink_Y: Drinker (reference: Non-drinker)\n")
cat("Drink_N: Non-drinker\n")
