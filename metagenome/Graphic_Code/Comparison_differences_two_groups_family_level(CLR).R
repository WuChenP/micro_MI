# ================================
# R脚本：CLR 转换 + Wilcoxon 两组比较 + 顶刊风格绘图
# ================================
...
# ====================== 3. CLR 转换 ========================
sample_cols <- c(mi_cols, con_cols)
data_matrix <- as.matrix(df[, sample_cols])

# 计算伪计数
nonzero_values <- data_matrix[data_matrix > 0]
if(length(nonzero_values) == 0) {
  stop("Error: No non-zero values found")
}

min_nonzero <- min(nonzero_values)
pseudocount <- min_nonzero / 2
print(paste("Pseudocount used:", pseudocount))

# 执行CLR转换
clr_matrix <- clr(data_matrix + pseudocount)
clr_data <- df
clr_data[, sample_cols] <- as.data.frame(clr_matrix)

# ====================== 4. Wilcoxon 检验 ========================
results <- map_df(1:nrow(df), function(i){
  fam <- df$Family[i]
  mi_vals  <- unlist(clr_data[i, mi_cols])
  con_vals <- unlist(clr_data[i, con_cols])
...
# ====================== 6. 绘制顶刊风格差异Family柱状图 ========================
output_folder <- "E:/Python/MI_Analysis/metagenome/Graphic/Comparison_differences_two_groups_family_level(CLR)"
if(!dir.exists(output_folder)) dir.create(output_folder, recursive = TRUE)

# 保存完整结果
write.csv(results, file.path(output_folder, "virus_family_CLR_wilcoxon_results.csv"), 
          row.names = FALSE, na = "")
...
if(nrow(sig_results) > 0) {
  ...
  annotation_data <- plot_data %>%
    group_by(Family) %>%
    summarise(
      max_abundance = max(CLR_Abundance),
      .groups = "drop"
    ) %>%
    left_join(sig_annotations, by = "Family")
  
  # 顶刊风格箱线图
  p1 <- ggplot(plot_data, aes(x = Family, y = CLR_Abundance, fill = Group)) +
    geom_boxplot(
      width = 0.6,
      alpha = 0.8,
      outlier.shape = 21,
    ...
    labs(
      ...
      subtitle = "CLR-transformed abundance with Wilcoxon test",
      x = "Viral Family",
  ...
  mean_data <- plot_data %>% 
    ...
    summarise(
      Mean_Abundance = mean(CLR_Abundance),
      SE = sd(CLR_Abundance) / sqrt(n()),
      .groups = "drop"
    )
  ...
  p2 <- ggplot(mean_data, aes(x = Family, y = Mean_Abundance, fill = Group)) +
    ...
    labs(
      ...
      subtitle = "CLR-transformed abundance with standard error",
      x = "Viral Family",
      y = "Mean CLR Abundance ± SE"
    ) +
    theme_classic(base_size = 12) +
    theme(
      axis.text.x = element_text(
