...
if(nrow(otu_filtered) < 3) {
  ...
  # 创建颜色映射
  ami_label <- paste0("AMI (n=", ami_count, ")")
  control_label <- paste0("Control (n=", control_count, ")")
  
  color_mapping <- c("#E64B35FF", "#4DBBD5FF")
  names(color_mapping) <- c(ami_label, control_label)
  
  # 绘制PCoA图（在标题中包含样本数量信息）
  p_pcoa <- ggplot(pcoa_df, aes(PC1, PC2, color = Group_with_count)) +
  ...
  ggsave("Beta_PCoA.png", p_pcoa, width = 7, height = 5.5, dpi = 300)
  cat("✅ Beta PCoA 图已保存\n")
